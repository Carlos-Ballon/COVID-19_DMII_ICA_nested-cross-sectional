...
format: 
  html:
    ...
    code-fold: true
    code-tools: 
      source: "https://github.com/Carlos-Ballon/COVID-19_DMII_ICA_Case-Control"
    theme: 
     light: flatly
     dark: darkly
...
tbl_1.3 <- data_match |>
  ...
  add_overall() |>
  add_p(frecuencia_cardiaca ~ "t.test",
        test.args = frecuencia_cardiaca ~ list(var.equal = TRUE)) |>
  bold_p(t = 0.05) |>
  modify_header(all_stat_cols() ~ "**{level}** (n = {n})")
```
...
tbl_3.6 <-
  ...
  add_p(frecuencia_cardiaca ~ "t.test",
        test.args = frecuencia_cardiaca ~ list(var.equal = FALSE)) |>
  bold_p(t = 0.05) |>
  modify_header(all_stat_cols() ~ "**{level}** (n = {n})")
```
...
```
> 📝***NOTE:*** The backward selection yielded the same results as the complete model
...
abline(h = c(-0.2, 0.2), lty = 2)
plot(dfbetas.dm[, "frecuencia_cardiaca.c>= 100"], ylab = "Heart rate >= 100")
abline(h = c(-0.2, 0.2), lty = 2)
plot(dfbetas.dm[, "neutrofilos.c> 6.3"], ylab = "Neutrophils > 6.3")
abline(h = c(-0.2, 0.2), lty = 2)
plot(dfbetas.dm[, "saturacion_de_oxigeno.c< 90"], ylab = "SaO2 < 90")
abline(h = c(-0.2, 0.2), lty = 2)
plot(dfbetas.dm[, "pafi.c<= 200"], ylab = "PaO2:FiO2 ratio <= 200")
abline(h = c(-0.2, 0.2), lty = 2)
```
...
```{r}
# Comparison of the regression coefficients and their standard errors
car::compareCoefs(final_model_dm,
                  sens_model_outlier_dm,
                  pvals = TRUE)
```
...
# Impact on the overall multiple degrees of freedom significance test
car::Anova(final_model_dm, type = 3)
car::Anova(sens_model_outlier_dm, type = 3)
```
...
table_sens_outlier_uv_dm <- subdat_dm |>
  tbl_uvregression(
    include = c(edad.c, hta, frecuencia_cardiaca.c, neutrofilos.c,
                saturacion_de_oxigeno.c, pafi.c),
    y = outcome,
    method = glm,
    method.args = list(family = binomial),
...
table_sens_outlier_mv_dm <-
  glm(
    outcome ~ edad.c + hta + frecuencia_cardiaca.c + neutrofilos.c +
      saturacion_de_oxigeno.c + pafi.c,
    family = binomial(link = "logit"),
    data = subdat_dm
  ) |>
...
abline(h = c(-0.2, 0.2), lty = 2)
plot(dfbetas.non[, "frecuencia_cardiaca.c>= 100"], ylab = "Heart rate >= 100")
abline(h = c(-0.2, 0.2), lty = 2)
plot(dfbetas.non[, "neutrofilos.c> 6.3"], ylab = "Neutrophils > 6.3")
abline(h = c(-0.2, 0.2), lty = 2)
plot(dfbetas.non[, "saturacion_de_oxigeno.c< 90"], ylab = "SaO2 < 90")
abline(h = c(-0.2, 0.2), lty = 2)
plot(dfbetas.non[, "pafi.c<= 200"], ylab = "PaO2:FiO2 ratio <= 200")
abline(h = c(-0.2, 0.2), lty = 2)
```
...
```{r}
# Comparison of the regression coefficients and their standard errors
car::compareCoefs(final_model_non,
                  sens_model_outlier_non,
                  pvals = TRUE)
```
...
# Impact on the overall multiple degrees of freedom significance test
car::Anova(final_model_non, type = 3)
car::Anova(sens_model_outlier_non, type = 3)
```
...
table_sens_outlier_uv_non <- subdat_non |>
  tbl_uvregression(
    include = c(edad.c, hta, disnea, frecuencia_cardiaca.c, neutrofilos.c,
                saturacion_de_oxigeno.c, pafi.c),
    y = outcome,
    method = glm,
    method.args = list(family = binomial),
...
::: {style="text-align: justify"}
We performed a sensitivity analysis to categorization of continuous data and our results did not change significantly except for dyspnea which became an independent predictor in T2DM patients, and the neutrophil did so for non-T2DM patients (Table S2). In addition, we performed a sensitivity analysis to outliers and influential observations and we identified 21 outliers and influential observations in T2DM patients and eight in non-T2DM patients. In patients with T2DM, all variables became independent predictors with the exception of PaO~2~:FiO~2~ ratio and dyspnea, the latter was not analyzed due to its overestimation. In patients without T2DM, all variables became independent predictors with the exception of hypertension (Table S3). While we still should focus on effect sizes rather than p-values, some of these changes are notably large.
:::
...
# Cos2 plot for the variables on the dimensions 1
x1 <- fviz_cos2(new_dim.dm, choice = "var", axes = 1)

# Cos2 plot for the variables on the dimensions 2
x2 <- fviz_cos2(new_dim.dm, choice = "var", axes = 2)

# Cos2 plot for the variables on the dimensions 3
x3 <- fviz_cos2(new_dim.dm, choice = "var", axes = 3)

# Cos2 plot for the variables on the dimensions 4
x4 <- fviz_cos2(new_dim.dm, choice = "var", axes = 4)

# Arrange multiple plots
cos2_plot <- ggpubr::ggarrange(x1, x2, x3, x4,
                               ncol = 2, nrow = 2)

annotate_figure(cos2_plot,
                top = text_grob(
...
x1 <- fviz_contrib(new_dim.dm,
                   choice = "var",
                   axes = 1,
                   top = 10)

# Contribution plot of the top 10 variables on PC2
x2 <- fviz_contrib(new_dim.dm,
                   choice = "var",
                   axes = 2,
                   top = 10)

# Contribution plot of the top 10 variables on PC3
x3 <- fviz_contrib(new_dim.dm,
                   choice = "var",
                   axes = 3,
                   top = 10)

# Contribution plot of the top 10 variables on PC4
x4 <- fviz_contrib(new_dim.dm,
                   choice = "var",
                   axes = 4,
                   top = 10)

# Arrange multiple plots
contrib_plot <- ggpubr::ggarrange(x1, x2, x3, x4,
                                  ncol = 2, nrow = 2)

annotate_figure(contrib_plot,
                top = text_grob(
...
annotate_figure(pca_dm,
                top = text_grob("PCA in patients with T2DM",
                                face = "bold",
                                size = 16))
```
...
```{r fig.height=8, fig.width=10, fig.align='center'}
# Cos2 plot for the variables on dimensions 1
x5 <- fviz_cos2(new_dim.non, choice = "var", axes = 1)

# Cos2 plot for the variables on dimensions 2
x6 <- fviz_cos2(new_dim.non, choice = "var", axes = 2)

# Cos2 plot for the variables on dimensions 3
x7 <- fviz_cos2(new_dim.non, choice = "var", axes = 3)

# Cos2 plot for the variables on dimensions 4
x8 <- fviz_cos2(new_dim.non, choice = "var", axes = 4)

# Arrange multiple plots
contrib_plot <- ggpubr::ggarrange(x5, x6, x7, x8,
                                  ncol = 2, nrow = 2)
...
```{r fig.height=8, fig.width=10, fig.align='center'}
# Contribution plot of the top 10 variables on PC1
x5 <- fviz_contrib(new_dim.non,
                   choice = "var",
                   axes = 1,
                   top = 10)

# Contribution plot of the top 10 variables on PC2
x6 <- fviz_contrib(new_dim.non,
                   choice = "var",
                   axes = 2,
                   top = 10)

# Contribution plot of the top 10 variables on PC3
x7 <- fviz_contrib(new_dim.non,
                   choice = "var",
                   axes = 3,
                   top = 10)

# Contribution plot of the top 10 variables on PC4
x8 <- fviz_contrib(new_dim.non,
                   choice = "var",
                   axes = 4,
                   top = 10)
...
annotate_figure(pca_non,
                top = text_grob("PCA in patients without T2DM",
                                face = "bold",
                                size = 16))
```
...
save_as_docx(
  flex_table_s3,
  path = stringr::str_glue(outputs_dir, "Table_s3.docx"),
  align = "center")
```
