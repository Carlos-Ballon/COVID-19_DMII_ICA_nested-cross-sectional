...
```
> 📝***NOTE:*** The forward selection yielded the same results as the complete model
...
::: {style="text-align: justify"}
We performed a sensitivity analysis to categorization of continuous data and our results were similar to those shown in the main analyses, except for dyspnea which became an independent predictor in T2DM patients, and the neutrophil did so for non-T2DM patients (Table S2). In addition, we performed a sensitivity analysis to outliers and influential observations and we identified 21 outliers and influential observations in T2DM patients and eight in non-T2DM patients. In patients with T2DM, all variables became independent predictors with the exception of PaO~2~:FiO~2~ ratio and dyspnea, the latter was not analyzed due to its overestimation. In patients without T2DM, all variables became independent predictors with the exception of hypertension (Table S3). While we still should focus on effect sizes rather than p-values, some of these changes are notably large.
:::
...
ind_plot.dm_1 <- fviz_pca_ind(
  ...
  ggtheme = theme_pomological(base_family = "Homemade Apple")
)
...
ind_plot_dm <- ggpubr::ggarrange(
  ...
  legend = "bottom",
  common.legend = TRUE,
  font.label = list(family = "Homemade Apple")) + 
  ggplot2::theme(panel.background = element_rect(fill = "#FFFEEA", color = NA))

paint_pomological(ind_plot_dm, width = 1000, height = 500)
```
...
paint_pomological(ind_plot_non, width = 1000, height = 500)
```
...
```{r fig.height=6, fig.width=14, fig.align='center', echo=FALSE}
# Arrange multiple plots
...
paint_pomological(tSNE_plot.dm, res = 110)
```
...
paint_pomological(tSNE_plot.non, res = 110)
```
